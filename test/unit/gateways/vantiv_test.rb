...
class VantivTest < Test::Unit::TestCase
  ...
  class AuthorizationRequestBuilderTest < self
    def test_capture
    ...
    def test_capture_without_amount
      stub_commit do |_, data, _|
        assert_match %r(<capture .*</capture>)m, data
        assert_match %r(<litleTxnId>100000000000000001</litleTxnId>), data
    ...
    def test_refund_with_descriptor
      stub_commit do |_, data, _|
        assert_match %r(<credit .*</credit>)m, data
        assert_match(
    ...
    def test_void_credit_card_purchase
      stub_commit do |_, data, _|
        assert_match %r(<void .*</void>)m, data
  ...
  class CheckRequestBuilderTest < self
    def test_purchase
      stub_commit do |_, data, _|
    ...
    def test_purchase_without_check_number
      @check.number = nil

      stub_commit do |_, data, _|
        assert_match %r(<echeckSale .*</echeckSale>)m, data
        assert_match %r(<echeck>.*</echeck>)m, data
        assert_no_match %r(<checkNum>), data
      end
  ...
  class CheckTokenRequestBuilderTest < self
    def test_purchase
      stub_commit do |_, data, _|
        assert_match %r(<echeckSale .*</echeckSale>)m, data
        assert_match %r(<orderId>this-must-be-truncated--</orderId>), data
  ...
  class CreditCardRequestBuilderTest < self
    def test_authorize
      stub_commit do |_, data, _|
    ...
    def test_authorize_with_debt_repayment
      stub_commit do |_, data, _|
        assert_match %r(<authorization .*</authorization>)m, data
    ...
    def test_authorize_with_order_source
      stub_commit do |_, data, _|
        assert_match %r(<authorization .*</authorization>)m, data
    ...
    def test_authorize_with_track_data
    ...
    def test_purchase_with_apple_pay_order_source
      stub_commit do |_, data, _|
    ...
    def test_purchase_with_billing_address
      stub_commit do |_, data, _|
        assert_match %r(<sale .*</sale>)m, data
        assert_match(
    ...
    def test_purchase_with_descriptor
      stub_commit do |_, data, _|
        assert_match %r(<sale .*</sale>)m, data
        assert_match %r(<customBilling>.*<descriptor>Name</descriptor>)m, data
        assert_match %r(<customBilling>.*<phone>Phone</phone>)m, data
    ...
    def test_purchase_with_order_source
      stub_commit do |_, data, _|
        assert_match %r(<sale .*</sale>)m, data
        assert_match %r(<orderSource>some-order-source</orderSource>), data
      end
    ...
    def test_purchase_with_track_data
      @credit_card.track_data = "credit-card-track-data"
  ...
  class CreditCardTokenRequestBuilderTest < self
    def test_authorize
      stub_commit do |_, data, _|
        assert_match %r(<authorization .*</authorization>)m, data
        assert_match %r(<orderId>this-must-be-truncated--</orderId>), data
  ...
  class RegistrationRequestBuilderTest < self
    def test_authorize
      stub_commit do |_, data, _|
        assert_match %r(<authorization .*</authorization>)m, data
        assert_match %r(<orderId>this-must-be-truncated--</orderId>), data
  ...
  class ResponseTest < self
    class AuthorizeTest < self
      def test_credit_card_failed
    ...
    class CaptureTest < self
      ...
      def test_credit_card_successful
        response = stub_comms do
          @gateway.authorize(@amount, @credit_card)
    ...
    class StoreTest < self
      def test_authorization_unsupported
        assert_raise NotImplementedError do
    ...
    class VerifyTest < self
      ...
      def test_credit_card_with_failed_void_successful
        response = stub_comms do
          @gateway.verify(@credit_card, @options)
        end.respond_with(
          _response_authorize__credit_card_successful,
    ...
    class VoidTest < self
      ...
      def test_refund_authorization_failed
        response = stub_comms do
      ...
      def test_refund_authorization_successful
        refund = stub_comms do
  ...
  class XmlTest < self
    ...
    def test_request_with_name_from_billing_address
      stub_commit do |_, data, |
        assert_match %r(<billToAddress>.*</billToAddress>)m, data
    ...
    # The values of the nodes below `<pos>` are the same regardless of the
    # action or the payment method. (Probably always a credit card with track
    # data).
    def test_request_with_pos
      @credit_card.track_data = "credit-card-track-data"
    ...
    def test_request_with_transaction_attributes
      stub_commit do |_, data, |
    ...
    def test_schema_validation_unsuccessful
      response = stub_comms do
        @gateway.store(@credit_card)
